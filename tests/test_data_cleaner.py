...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        df_clean = cleaner.drop_invalid_rows(df, ["name", "age"])
        ...
        self.assertEqual(df_clean["name"].isna().sum(),0)
        self.assertEqual(df_clean["age"].isna().sum(),0)
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        df_clean = cleaner.trim_strings(df, ["name"])
        ...
        self.assertEqual(df_clean.loc[0, "name"], "Alice")
        ...
        pdt.assert_series_equal(df["city"], df_clean["city"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        #- Llamar a remove_outliers_iqr con la columna "age" y factor=1.5 # Corregido factor a 0.5 #
        df_clean = cleaner.remove_outliers_iqr(df, "age", factor=0.5)
        ...
        self.assertNotIn(120, df_clean["age"].values)
        ...
        self.assertIn(25, df_clean["age"].values)
        self.assertIn(35, df_clean["age"].values)
